...
public class AuthorCommand {
    ...
    * @return the author command
    */
    public Future<AuthorList> authorGetSummaryListCommand(Long loginId, String sortExpression, String filterCondition, Long skip, Long pageSize) {
        LOGGER.info("info: authorGetSummaryListCommand - start");
        Promise<AuthorList> promise = Promise.promise();
        client
            ...
            .onSuccess(res -> {
                ...
                try{
                    ...
                    for (Row row : res) {
                        Author author = createAuthor(row);
                        LOGGER.info("info: handle query authorGetSummaryListCommand result - ok");
                        authorList.add(author);
                    }
                    ...
                    if (promise.tryComplete()) {
                      LOGGER.info("info: handle query authorGetSummaryListCommand result - no result");
                    }
                } catch(Exception e){
                    LOGGER.info("info: handle query authorGetSummaryListCommand result - no result");
                    promise.fail(e);
            ...
            .onFailure(ar -> {
                LOGGER.error("info: handle query authorGetSummaryListCommand result - failed");
                promise.fail(ar);
            });
        return promise.future();
    }
